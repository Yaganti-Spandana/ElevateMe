...
import "../css/Black_&_GoldResumeEditor.css";
...
const BlackGoldResumeEditor = () => {
  const templateId = "Black_and_Gold";
...
const splitIntoPages = useCallback((container) => {
  ...
  const leftSections = Array.from(
    container.querySelectorAll(".modern-left6 .section-block")
  );

  const rightSections = Array.from(
    container.querySelectorAll(".modern-right6 .side-section6")
  );
  ...
  // =========================
  useEffect(() => {
  const saved = localStorage.getItem("Black_and_Gold");
  if (saved) {
...
useEffect(() => {
  ...
  // =========================
   const handleDownload = async () => {
    ...
    const pages = resumeRef.current.querySelectorAll(".pdf-page6");
  ...
  const ResumeInner = () => (
    <div className="modern-grid6">
      <main className="modern-left6">
        <section className="section-block">
        <div className="flex-black">
            <h1 className="modern-name6">{common.name}{ }</h1>
    <h1 className="modern-title6">{common.title}</h1></div>
    </section>
        {/* Summary */}
        <section className="section-block">
          <h3 className="section-title6">Summary</h3>
        ...
        {experience.length > 0 && (
          <section className="section-block">
            <h3 className="section-title6">Experience</h3>
        ...
        {education.length > 0 && (
          <section className="section-block">
            <h3 className="section-title6">Education</h3>
            {education.map((edu, i) => (
              <div key={i} className="item-block">
      ...
      <aside className="modern-right6">
        <div className="side-section6">
    <div className="photo-circle6">
  {common.photo ? (
    <img
      src={common.photo}
      alt="profile"
    ...
    <div className="photo-placeholder6">Photo</div>
  )}
</div></div>
        ...
        {/* Projects */}
        {projects.length > 0 && (
          <section className="side-section6">
            <h3>Projects</h3>
            {projects.map((p, i) => (
              <div key={i} className="item-block">
...
window.open(`/portfolio/Black_and_Gold/${portfolioId}`, "_blank");
ReactGA.event({
    category: "Portfolio",
...
Create Portfolio Link
  ...
  {pages.map((pageSections, pageIndex) => (
    <div key={pageIndex} className="pdf-page6">
      <div className="resume-content modern-grid6">
        ...
        <aside className="modern-right6">
          {pageSections.filter(s => s.classList.contains('side-section6')).map((section, idx) => (
            <div key={idx} dangerouslySetInnerHTML={{ __html: section.outerHTML }} />
          ))}
        </aside>
      </div>
...
export default BlackGoldResumeEditor;