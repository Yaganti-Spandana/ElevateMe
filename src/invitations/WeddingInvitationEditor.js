...
import weddingInvitationElements from "../data/weddingInvitationData";
...
export default function WeddingInvitationEditor() {

  const stageRef = useRef();
  const transformerRef = useRef();

  const [elements, setElements] = useState(weddingInvitationElements);
  const [selectedId, setSelectedId] = useState(null);

  const [font, setFont] = useState("Arial");
  const [color, setColor] = useState("#000");
  const [bgColor, setBgColor] = useState("#c3c1c1");
...
>
  ...
  fillLinearGradientEndPoint={{ x: 595, y: 842 }}
  fillLinearGradientColorStops={[
    0, "#989797",   // light top
    1, "#ffffff"    // darker bottom
  ]}
/>
            )}
